// © 2016 Mario Lelas // Contributed by Halil Emre Yildiz 2022
...
Shader "DoubleSided/TwoFace/TwoFaceCutoff"
{
	...
	SubShader
	{
		...
		void surf(Input IN, inout SurfaceOutput o)
		{
			...
			if (_sign < 0) { clip(backTex.a - _CutoffBack); }
			else { clip(frontTex.a - _CutoffFront); }
			float value = (_sign + 1.0f) / 2.0f;

			float4 alb = lerp(backTex, frontTex, value);
			o.Albedo = alb.rgb * _Color.rgb * (_BrightnessV * 4);
	...
	Fallback "Mobile/VertexLit" // "DoubleSided/TwoFace/TwoFaceVertexLitCutoutCullOff" // "DoubleSided/Legacy Shaders/VertexLitCullOff" 
}
